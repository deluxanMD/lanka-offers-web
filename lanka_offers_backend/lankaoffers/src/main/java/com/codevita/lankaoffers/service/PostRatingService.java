...
import com.codevita.lankaoffers.repository.PostRatingRepo;
 ...
 */
@Service
public class PostRatingService implements EntityManagementService {
	
	@Autowired
	private PostRatingRepo postRatingRepo;
	
	private static Logger logger = LogManager.getLogger(PostRatingRepo.class);

	private static String thisServiceForLogger = "PostRating Service ";

	@Override
	public List<PostRating> getAll(Pagination pagination, Map<String, String> filters, String keyword) {
		...
		try {
			List<PostRating> dataList = postRatingRepo.findAll();
			logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_OPERATION_SUCCESSFUL);
//			return (List<?>) new GenericResult(true, AppConstants.MSG_SUCCESS, AppConstants.HTTP_RESULT_SUCCESS, postRatings);
			return dataList;
		} catch (Exception ex) {
...
//			return (List<?>) new GenericResult(false, "Failer Error - "+ ex.getMessage(), AppConstants.HTTP_RESULT_FAILED, null);
	...
	public GenericResult getOne(Long id) {
		...
		logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_SERVICE_STARTED);
		try {
			Optional<PostRating> postRating = postRatingRepo.findById(id);
			boolean isExist = postRating != null;
			if(isExist) {
				logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_OPERATION_SUCCESSFUL);
				return new GenericResult(true, AppConstants.MSG_SUCCESS, AppConstants.HTTP_RESULT_SUCCESS, postRating);
			}else {
	...
	public GenericResult store(Object obj) {
		...
		logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_SERVICE_STARTED);
		try {
			PostRating postRating = (PostRating)obj;
			postRating = postRatingRepo.save(postRating);
			logger.info(thisMethodInfoForLogger + AppConstants.LOGGER_MSG_OPERATION_SUCCESSFUL);
			return new GenericResult(true, AppConstants.MSG_SUCCESS, AppConstants.HTTP_RESULT_SUCCESS, postRating);
		} catch (Exception ex) {
			logger.error(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_OPERATION_FAILED + " - " + ex.getMessage());
	...
	public GenericResult update(Object obj) {
		...
		try {
			PostRating postRating = (PostRating)obj;
			Optional<PostRating> postRatingFromDb = postRatingRepo.findById(postRating.getId());
			boolean isExist = postRatingFromDb != null;
			if(isExist) {
				postRating = postRatingRepo.save(postRating);
				logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_OPERATION_SUCCESSFUL);
				return new GenericResult(true, AppConstants.MSG_SUCCESS, AppConstants.HTTP_RESULT_SUCCESS, postRating);

...
import com.codevita.lankaoffers.repository.PostCategoryRepo;
 ...
 */
@Service
public class PostCategoryService implements EntityManagementService {

	@Autowired
	private PostCategoryRepo postCategoryRepo;
	
	private static Logger logger = LogManager.getLogger(PostCategoryRepo.class);
	
	private static String thisServiceForLogger = "PostCategory Service ";

	@Override
	public List<PostCategory> getAll(Pagination pagination, Map<String, String> filters, String keyword) {
		...
		try {
			List<PostCategory> dataList = postCategoryRepo.findAll();
			logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_OPERATION_SUCCESSFUL);
//			return (List<?>) new GenericResult(true, AppConstants.MSG_SUCCESS, AppConstants.HTTP_RESULT_SUCCESS, postCategories);
			return dataList;
		} catch (Exception ex) {
...
//			return (List<?>) new GenericResult(false, "Failer Error - "+ ex.getMessage(), AppConstants.HTTP_RESULT_FAILED, null);
	...
	public GenericResult getOne(Long id) {
		...
		logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_SERVICE_STARTED);
		try {
			Optional<PostCategory> postCategory = postCategoryRepo.findById(id);
			boolean isExist = postCategory != null;
			if(isExist) {
				logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_OPERATION_SUCCESSFUL);
				return new GenericResult(true, AppConstants.MSG_SUCCESS, AppConstants.HTTP_RESULT_SUCCESS, postCategory);
			}else {
	...
	public GenericResult store(Object obj) {
		...
		try {
			...
			postCategory = postCategoryRepo.save(postCategory);
			logger.info(thisMethodInfoForLogger + AppConstants.LOGGER_MSG_OPERATION_SUCCESSFUL);
			return new GenericResult(true, AppConstants.MSG_SUCCESS, AppConstants.HTTP_RESULT_SUCCESS, postCategory);
		} catch (Exception ex) {
			logger.error(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_OPERATION_FAILED + " - " + ex.getMessage());
	...
	public GenericResult update(Object obj) {
		...
		try {
			PostCategory postCategory = (PostCategory)obj;
			Optional<PostCategory> postCategoryFromDb = postCategoryRepo.findById(postCategory.getId());
			boolean isExist = postCategoryFromDb != null;
			if(isExist) {
				postCategory = postCategoryRepo.save(postCategory);
				logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_OPERATION_SUCCESSFUL);
				return new GenericResult(true, AppConstants.MSG_SUCCESS, AppConstants.HTTP_RESULT_SUCCESS, postCategory);

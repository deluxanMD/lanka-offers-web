...
import com.codevita.lankaoffers.repository.PostShareRepo;
 ...
 */
@Service
public class PostShareService implements EntityManagementService {

	@Autowired
	private PostShareRepo postShareRepo;
	
	private static Logger logger = LogManager.getLogger(PostShareRepo.class);
	
	private static String thisServiceForLogger = "PostShare Service ";

	@Override
	public List<PostShare> getAll(Pagination pagination, Map<String, String> filters, String keyword) {
		...
		try {
			List<PostShare> dataList = postShareRepo.findAll();
			logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_OPERATION_SUCCESSFUL);
//			return (List<?>) new GenericResult(true, AppConstants.MSG_SUCCESS, AppConstants.HTTP_RESULT_SUCCESS, postShares);
			return dataList;
		} catch (Exception ex) {
...
//			return (List<?>) new GenericResult(false, "Failer Error - "+ ex.getMessage(), AppConstants.HTTP_RESULT_FAILED, null);
	...
	public GenericResult getOne(Long id) {
		...
		logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_SERVICE_STARTED);
		try {
			Optional<PostShare> postShare = postShareRepo.findById(id);
			boolean isExist = postShare != null;
			if(isExist) {
				logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_OPERATION_SUCCESSFUL);
				return new GenericResult(true, AppConstants.MSG_SUCCESS, AppConstants.HTTP_RESULT_SUCCESS, postShare);
			}else {
	...
	public GenericResult store(Object obj) {
		...
		try {
			...
			postShare = postShareRepo.save(postShare);
			logger.info(thisMethodInfoForLogger + AppConstants.LOGGER_MSG_OPERATION_SUCCESSFUL);
			return new GenericResult(true, AppConstants.MSG_SUCCESS, AppConstants.HTTP_RESULT_SUCCESS, postShare);
		} catch (Exception ex) {
			logger.error(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_OPERATION_FAILED + " - " + ex.getMessage());
	...
	public GenericResult update(Object obj) {
		...
		try {
			PostShare postShare = (PostShare)obj;
			Optional<PostShare> postShareFromDb = postShareRepo.findById(postShare.getId());
			boolean isExist = postShareFromDb != null;
			if(isExist) {
				postShare = postShareRepo.save(postShare);
				logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_OPERATION_SUCCESSFUL);
				return new GenericResult(true, AppConstants.MSG_SUCCESS, AppConstants.HTTP_RESULT_SUCCESS, postShare);

...
import com.codevita.lankaoffers.repository.PostLikeRepo;
 ...
 */
@Service
public class PostLikeService implements EntityManagementService {

	@Autowired
	private PostLikeRepo postLikeRepo;
	
	private static Logger logger = LogManager.getLogger(PostLikeRepo.class);
	
	private static String thisServiceForLogger = "PostLike Service ";

	@Override
	public List<PostLike> getAll(Pagination pagination, Map<String, String> filters, String keyword) {
		...
		try {
			List<PostLike> dataList = postLikeRepo.findAll();
			logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_OPERATION_SUCCESSFUL);
//			return (List<?>) new GenericResult(true, AppConstants.MSG_SUCCESS, AppConstants.HTTP_RESULT_SUCCESS, postLikes);
			return dataList;
		} catch (Exception ex) {
...
//			return (List<?>) new GenericResult(false, "Failer Error - "+ ex.getMessage(), AppConstants.HTTP_RESULT_FAILED, null);
	...
	public GenericResult getOne(Long id) {
		...
		logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_SERVICE_STARTED);
		try {
			Optional<PostLike> postLike = postLikeRepo.findById(id);
			boolean isExist = postLike != null;
			if(isExist) {
				logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_OPERATION_SUCCESSFUL);
				return new GenericResult(true, AppConstants.MSG_SUCCESS, AppConstants.HTTP_RESULT_SUCCESS, postLike);
			}else {
	...
	public GenericResult update(Object obj) {
		...
		try {
			PostLike postLike = (PostLike)obj;
			Optional<PostLike> postLikeFromDb = postLikeRepo.findById(postLike.getId());
			boolean isExist = postLikeFromDb != null;
			if(isExist) {
				postLike = postLikeRepo.save(postLike);
				logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_OPERATION_SUCCESSFUL);
				return new GenericResult(true, AppConstants.MSG_SUCCESS, AppConstants.HTTP_RESULT_SUCCESS, postLike);

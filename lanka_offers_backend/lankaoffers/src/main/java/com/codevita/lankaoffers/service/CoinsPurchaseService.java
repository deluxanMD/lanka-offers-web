...
import com.codevita.lankaoffers.repository.CoinsPurchaseRepo;
 ...
 */
@Service
public class CoinsPurchaseService implements EntityManagementService {
	
	@Autowired
	private CoinsPurchaseRepo coinsPurchaseRepo;
	
	private static Logger logger = LogManager.getLogger(CoinsPurchaseRepo.class);
	
	private String thisServiceForLogger = "CoinsPurchase Service";

	@Override
	public List<CoinsPurchase> getAll(Pagination pagination, Map<String, String> filters, String keyword) {
		...
		try {
			List<CoinsPurchase> dataList = coinsPurchaseRepo.findAll();
			logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_OPERATION_SUCCESSFUL);
//			return (List<?>) new GenericResult(true, AppConstants.MSG_SUCCESS, AppConstants.HTTP_RESULT_SUCCESS, coinsPurchases);
			return dataList;
		} catch (Exception ex) {
...
//			return (List<?>) new GenericResult(false, "Failer Error - "+ ex.getMessage(), AppConstants.HTTP_RESULT_FAILED, null);
	...
	public GenericResult getOne(Long id) {
		...
		logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_SERVICE_STARTED);
		try {
			Optional<CoinsPurchase> coinsPurchase = coinsPurchaseRepo.findById(id);
			boolean isExist = coinsPurchase != null;
			if(isExist) {
				logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_OPERATION_SUCCESSFUL);
				return new GenericResult(true, AppConstants.MSG_SUCCESS, AppConstants.HTTP_RESULT_SUCCESS, coinsPurchase);
			}else {
	...
	public GenericResult store(Object obj) {
		...
		logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_SERVICE_STARTED);
		try {
			CoinsPurchase coinsPurchase = (CoinsPurchase)obj;
			coinsPurchase = coinsPurchaseRepo.save(coinsPurchase);
			logger.info(thisMethodInfoForLogger + AppConstants.LOGGER_MSG_OPERATION_SUCCESSFUL);
			return new GenericResult(true, AppConstants.MSG_SUCCESS, AppConstants.HTTP_RESULT_SUCCESS, coinsPurchase);
		} catch (Exception ex) {
			logger.error(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_OPERATION_FAILED + " - " + ex.getMessage());
	...
	public GenericResult update(Object obj) {
		...
		try {
			CoinsPurchase coinsPurchase = (CoinsPurchase)obj;
			Optional<CoinsPurchase> coinsPurchaseFromDb = coinsPurchaseRepo.findById(coinsPurchase.getId());
			boolean isExist = coinsPurchaseFromDb != null;
			if(isExist) {
				coinsPurchase = coinsPurchaseRepo.save(coinsPurchase);
				logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_OPERATION_SUCCESSFUL);
				return new GenericResult(true, AppConstants.MSG_SUCCESS, AppConstants.HTTP_RESULT_SUCCESS, coinsPurchase);
	...
	public GenericResult delete(Long id) {
		...
		try {
			...
			boolean isExist = coinsPurchase != null;
			if(isExist) {
				coinsPurchaseRepo.deleteById(id);
				logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_OPERATION_SUCCESSFUL);
				return new GenericResult(true, AppConstants.MSG_SUCCESS, AppConstants.HTTP_RESULT_SUCCESS, coinsPurchase);
			}else {
				logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_OPERATION_DATA_NOT_FOUND);

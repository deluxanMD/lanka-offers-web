...
import com.codevita.lankaoffers.service.CategoryPreferenceService;
...
public class CategoryPreferenceController {
	...
	public ResponseEntity<?> getAll(HttpServletRequest request) {
		...
		try {
			...
			List<CategoryPreference> dataList = categoryPreferenceService.getAll(pagination, filters, null);
	...
	@GetMapping(AppConstants.CATEGORY_PREFERENCE_BY_ID_URL)
	public ResponseEntity<?> getOne(HttpServletRequest request, @PathVariable long id) {
		GenericResult result = new GenericResult();
		try {
			result = categoryPreferenceService.getOne(id);
	...
	@PostMapping(AppConstants.CATEGORY_PREFERENCE_URL)
	public ResponseEntity<Object> store(@RequestBody CategoryPreference categoryPreference) {
		GenericResult result = new GenericResult();
		try {
			// validation
			validateResult = Validator.validateNumber(categoryPreference.getId(), "Id");
			if (!validateResult.isSuccess()) {
				result = validateResult;
				return new ResponseEntity<>(result, HttpStatus.BAD_REQUEST);
			}

			result = categoryPreferenceService.store(categoryPreference);
	...
	@PutMapping(AppConstants.CATEGORY_PREFERENCE_BY_ID_URL)
	public ResponseEntity<?> update(@RequestBody CategoryPreference categoryPreference, @PathVariable long id) {
		GenericResult result = new GenericResult();
		try {
			// validation
			validateResult = Validator.validateNumber(categoryPreference.getId(), "Id");
			...
			result = categoryPreferenceService.update(categoryPreference);
	...
	@DeleteMapping(AppConstants.CATEGORY_PREFERENCE_BY_ID_URL)
	public ResponseEntity<?> delete(@PathVariable long id) {
		GenericResult result = new GenericResult();
		try {
			result = categoryPreferenceService.delete(id);

...
import com.codevita.lankaoffers.entity.PostComment;
import com.codevita.lankaoffers.repository.PostCommentRepo;
 ...
 */
@Service
public class PostCommentService implements EntityManagementService {
	
	@Autowired
	private PostCommentRepo postCommentRepo;
	
	private static Logger logger = LogManager.getLogger(PostCommentRepo.class);
	
	private static String thisServiceForLogger = "PostComment Service ";

	@Override
	public List<PostComment> getAll(Pagination pagination, Map<String, String> filters, String keyword) {
		...
		try {
			List<PostComment> dataList = postCommentRepo.findAll();
			logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_OPERATION_SUCCESSFUL);
//			return (List<?>) new GenericResult(true, AppConstants.MSG_SUCCESS, AppConstants.HTTP_RESULT_SUCCESS, postComments);
			return dataList;
		} catch (Exception ex) {
...
//			return (List<?>) new GenericResult(false, "Failer Error - "+ ex.getMessage(), AppConstants.HTTP_RESULT_FAILED, null);
	...
	public GenericResult getOne(Long id) {
		...
		logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_SERVICE_STARTED);
		try {
			Optional<PostComment> postComment = postCommentRepo.findById(id);
			boolean isExist = postComment != null;
			if(isExist) {
				logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_OPERATION_SUCCESSFUL);
				return new GenericResult(true, AppConstants.MSG_SUCCESS, AppConstants.HTTP_RESULT_SUCCESS, postComment);
			}else {
	...
	public GenericResult update(Object obj) {
		...
		try {
			PostComment postComment = (PostComment)obj;
			Optional<PostComment> postCommentFromDb = postCommentRepo.findById(postComment.getId());
			boolean isExist = postCommentFromDb != null;
			if(isExist) {
				postComment = postCommentRepo.save(postComment);
				logger.info(thisMethodInfoForLogger, AppConstants.LOGGER_MSG_OPERATION_SUCCESSFUL);
				return new GenericResult(true, AppConstants.MSG_SUCCESS, AppConstants.HTTP_RESULT_SUCCESS, postComment);
